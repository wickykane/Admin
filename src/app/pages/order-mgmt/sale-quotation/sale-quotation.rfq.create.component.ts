...
export class SaleQuotationRfqCreateComponent implements OnInit {
  ...
  constructor(private vRef: ViewContainerRef, private fb: FormBuilder, public toastr: ToastrService, private router: Router, private modalService: NgbModal, private orderService: OrderService) {
    this.generalForm = fb.group({
      'sale_quote_num': [{value:null,disabled:true}, Validators.required],
      'rfq_id': [{value:null,disabled:true}],
      'quote_date': [null, Validators.required],
      'expiry_date': [null, Validators.required],
      'sale_executive_name': [{value:null,disabled:true}],
      'ref_num':[]     
    });
  }
  ...
  ngOnInit() {
    ...
    this.getBuyerType();
    this.generalForm.patchValue({'sale_executive_name':this.user.full_name}) ;
  }
  /**
   * Mater Data
   */
  ...
  generateSaleQuote(){
    this.orderService.getSalesQuotation().subscribe((res)=>{
        this.generalForm.patchValue({'sale_quote_num':res.results.code}) ;
    })
}
